...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        const declarationIds = declarations?.data[1].map(declaration => declaration.id);
        const deleteDeclarationPromises = declarationIds.map(declarationId => caller.declarations.removeDeclaration({
            declarationId,
        }));
